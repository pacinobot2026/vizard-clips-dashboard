...
export default function BusinessBoard() {
  ...
  const [newCard, setNewCard] = useState({ title: '', description: '', labels: [], column: '' });
  ...
  const handleDeleteResource = async (resourceId) => {
    const updatedResources = selectedBusiness.resources.filter(r => r.id !== resourceId);
  ...
  const toggleLabel = (label) => {
    if (newCard.labels.includes(label)) {
      setNewCard({ ...newCard, labels: newCard.labels.filter(l => l !== label) });
    } else {
        ...
        <div style={{ maxWidth: '1600px', margin: '0 auto' }}>
          ...
          {showResources && selectedBusiness && (
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <h3 style={{ color: '#fff', margin: 0, fontSize: '18px' }}>📎 Resources for {selectedBusiness.name}</h3>
                <button
              ...
              {resources.length === 0 ? (
                <p style={{ color: '#9ca3af', margin: 0 }}>No resources yet. Add links, docs, or notes.</p>
              ) : (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
                  {resources.map(r => (
                    <div key={r.id} style={{
                      ...
                      <span>{r.type === 'link' ? '🔗' : r.type === 'doc' ? '📄' : '📝'}</span>
                      {r.url ? (
                        <a href={r.url} target="_blank" rel="noopener noreferrer" style={{ color: '#8b5cf6', textDecoration: 'none' }}>
                          {r.title}
                        </a>
                      ) : (
              ...
              {/* Add Resource Modal */}
              {showAddResource && (
                <div style={{ marginTop: '16px', padding: '16px', background: '#111827', borderRadius: '8px' }}>
                  <input
                    type="text"
                  ...
                  <div style={{ display: 'flex', gap: '10px' }}>
                    ...
                    <button onClick={handleAddResource} style={{ background: '#10b981', border: 'none', borderRadius: '6px', padding: '10px 16px', color: '#fff', cursor: 'pointer' }}>
                      Add
                    </button>
                    <button onClick={() => setShowAddResource(false)} style={{ background: '#374151', border: 'none', borderRadius: '6px', padding: '10px 16px', color: '#fff', cursor: 'pointer' }}>
                      Cancel
                    </button>
                  </div>
                </div>
          ...
          {selectedBusiness ? (
              ...
              {columns.map(column => (
                ...
                >
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {cards.filter(c => c.column === column).map(card => (
                      <div
                        key={card.id}
                        draggable
                      ...
                      >
                          ...
                          <div style={{ display: 'flex', gap: '6px', marginBottom: '10px', flexWrap: 'wrap' }}>
                            {card.labels.map(label => (
                              <span
                                key={label}
                                style={{
                                  background: LABEL_COLORS[label]?.bg || '#6b7280',
                        ...
                        <h4 style={{ color: '#fff', margin: 0, fontSize: '14px', fontWeight: '500' }}>
                          {card.title}
                        </h4>
                        {card.description && (
                          <p style={{ color: '#9ca3af', margin: '8px 0 0', fontSize: '13px', lineHeight: '1.4' }}>
            ...
            <div style={{ textAlign: 'center', padding: '60px', color: '#9ca3af' }}>
              <p style={{ fontSize: '18px', marginBottom: '16px' }}>No business selected</p>
              <button
                onClick={() => setShowAddBusiness(true)}
                style={{
                  background: '#8b5cf6',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
              <button onClick={() => setShowAddBusiness(false)} style={{ background: '#374151', border: 'none', borderRadius: '8px', padding: '10px 20px', color: '#fff', cursor: 'pointer' }}>
                Cancel
              </button>
              <button onClick={handleAddBusiness} style={{ background: '#8b5cf6', border: 'none', borderRadius: '8px', padding: '10px 20px', color: '#fff', cursor: 'pointer', fontWeight: '600' }}>
                Add Business
              </button>
            </div>
          </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ marginBottom: '16px' }}>
              <p style={{ color: '#9ca3af', fontSize: '13px', marginBottom: '8px' }}>Labels:</p>
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {Object.keys(LABEL_COLORS).map(label => (
                  <button
                    ...
                    style={{
                      background: newCard.labels.includes(label) ? LABEL_COLORS[label].bg : '#374151',
                      color: newCard.labels.includes(label) ? LABEL_COLORS[label].text : '#9ca3af',
                      border: 'none',
                      borderRadius: '4px',
                      padding: '6px 12px',
                      fontSize: '12px',
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
              <button onClick={() => { setShowAddCard(null); setNewCard({ title: '', description: '', labels: [], column: '' }); }} style={{ background: '#374151', border: 'none', borderRadius: '8px', padding: '10px 20px', color: '#fff', cursor: 'pointer' }}>
                Cancel
              </button>
              <button onClick={handleAddCard} style={{ background: '#8b5cf6', border: 'none', borderRadius: '8px', padding: '10px 20px', color: '#fff', cursor: 'pointer', fontWeight: '600' }}>
                Add Card
              </button>
            </div>
          </div>
      ...
      {showEditCard && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ marginBottom: '16px' }}>
              <p style={{ color: '#9ca3af', fontSize: '13px', marginBottom: '8px' }}>Move to column:</p>
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {columns.map(col => (
                  <button
                    key={col}
                    onClick={() => setShowEditCard({ ...showEditCard, column: col })}
                    style={{
                      background: showEditCard.column === col ? '#8b5cf6' : '#374151',
                      color: '#fff',
                      border: 'none',
                      borderRadius: '6px',
                      padding: '8px 14px',
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (labels.includes(label)) {
                        setShowEditCard({ ...showEditCard, labels: labels.filter(l => l !== label) });
                      } else {
                        setShowEditCard({ ...showEditCard, labels: [...labels, label] });
                      }
                    }}
                    style={{
                      background: (showEditCard.labels || []).includes(label) ? LABEL_COLORS[label].bg : '#374151',
                      color: (showEditCard.labels || []).includes(label) ? LABEL_COLORS[label].text : '#9ca3af',
                      border: 'none',
                      borderRadius: '4px',
                      padding: '6px 12px',
                      fontSize: '12px',
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
              <button onClick={() => setShowEditCard(null)} style={{ background: '#374151', border: 'none', borderRadius: '8px', padding: '10px 20px', color: '#fff', cursor: 'pointer' }}>
                Cancel
              </button>
              <button 
                onClick={async () => {
                ...
                style={{ background: '#10b981', border: 'none', borderRadius: '8px', padding: '10px 20px', color: '#fff', cursor: 'pointer', fontWeight: '600' }}
              >
                Save Changes
              </button>
            </div>
