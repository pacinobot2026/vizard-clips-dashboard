...
const EMPTY_FORM = { title: '', url: '', description: '', category: '', status: 'read-later', tags: '' };
...
function Bookmarks() {
  ...
  useEffect(() => { if (session) loadBookmarks(); }, [session]);
  useEffect(() => { applyFilters(allBookmarks); }, [allBookmarks, filter, category, sortBy]);
  ...
  function openEdit(item) {
    setForm({ title: item.title, url: item.url || '', description: item.description || '', category: item.category || '', status: item.status, tags: (item.tags || []).join(', ') });
  ...
  const filteredBookmarks = bookmarks.filter(b =>
    searchTerm === '' || b.title?.toLowerCase().includes(searchTerm.toLowerCase()) || b.description?.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen">
        <NavigationSidebar />
        <div className="flex-1 flex items-center justify-center text-gray-400">Loading bookmarks…</div>
      </div>
    );
  }
      ...
      <main className="flex-1 p-8 pt-16 md:pt-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-3xl font-bold gradient-text mb-1">📑 Bookmarks</h1>
              <p className="text-sm text-gray-400">Save and organize useful links</p>
            </div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-4">
            {[{ icon: '📖', label: 'Read Later', key: 'read-later' }, { icon: '⭐', label: 'Favorites', key: 'favorites' }, { icon: '📦', label: 'Archived', key: 'archived' }].map(({ icon, label, key }) => (
          ...
          <div className="flex gap-3 flex-wrap mb-6">
            <input type="text" placeholder="🔍 Search bookmarks..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="flex-1 min-w-48 bg-gray-800/50 border border-gray-600/50 rounded-lg px-4 py-2.5 text-white text-sm outline-none focus:border-purple-500" />
          ...
          <div className="glass-card rounded-2xl p-6">
            {filteredBookmarks.length === 0 ? (
              ...
              <div className="grid gap-4" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))' }}>
                {filteredBookmarks.map(b => <BookmarkCard key={b.id} bookmark={b} onEdit={() => openEdit(b)} onDelete={() => setDeleteId(b.id)} />)}
              </div>
            )}
          </div>
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-lg">
            <h2 className="text-xl font-bold text-white mb-4">{modal.mode === 'add' ? 'Add Bookmark' : 'Edit Bookmark'}</h2>
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-sm">
            <h2 className="text-lg font-bold text-white mb-2">Delete Bookmark?</h2>
  ...
  return (
    <div className="group bg-gray-800/70 rounded-xl border border-gray-600/50 p-4 transition-all hover:border-purple-500/50 hover:-translate-y-1 hover:shadow-lg hover:shadow-purple-900/20">
      <div className="flex items-start justify-between gap-2 mb-2">
        <a href={bookmark.url} target="_blank" rel="noopener noreferrer" className="text-white hover:text-blue-400 text-base font-semibold block no-underline leading-snug transition-colors">
          {bookmark.title}
        </a>
      ...
      <p className="text-gray-400 text-sm mb-3 leading-relaxed">{bookmark.description}</p>
      <div className="flex gap-2 flex-wrap mb-3">
        {bookmark.category && <span className="px-2 py-1 bg-gray-900 rounded text-xs text-gray-400">{bookmark.category}</span>}
        <span className="px-2 py-1 bg-gray-900 rounded text-xs text-gray-400">{new Date(bookmark.created_at).toLocaleDateString()}</span>
      </div>
