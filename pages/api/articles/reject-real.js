// Reject article - updates status to REJECTED in Letterman
// Note: Letterman may not have a REJECTED state, so we might set to DRAFT with a tag
...
export default async function handler(req, res) {
  ...
  try {
    // Try to set to REJECTED state
    // If that doesn't exist, we'll set back to DRAFT and add a "rejected" keyword
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        state: 'REJECTED',
        keywords: ['rejected'] // Add rejected keyword for filtering
      })
    });
    ...
    if (!response.ok) {
      ...
      // If REJECTED state doesn't exist, try DRAFT with rejected keyword
      if (response.status === 400) {
        ...
        return res.status(200).json({ 
          ...
          note: 'Set to DRAFT with rejected keyword (REJECTED state not available)'
        });
      }
