...
const EMPTY_FORM = { title: '', url: '', description: '', category: '', status: 'to-buy', price: '', image: '', tags: '' };
...
function Shopping() {
  ...
  useEffect(() => { if (session) loadShopping(); }, [session]);
  useEffect(() => { applyFilters(allShopping); }, [allShopping, filter, category, sortBy]);

  async function loadShopping() {
    setLoading(true);
    try {
      const res = await fetch('/api/shopping', { headers: { Authorization: `Bearer ${session.access_token}` } });
  ...
  function applyFilters(source) {
    ...
    const categoryMap = {};
    source.forEach(b => { if (b.category) { categoryMap[b.category] = (categoryMap[b.category] || 0) + 1; } });
  ...
  function openEdit(item) {
    setForm({ title: item.title, url: item.url || '', description: item.description || '', category: item.category || '', status: item.status, price: item.price || '', image: item.image || '', tags: (item.tags || []).join(', ') });
  ...
  const filteredShopping = shopping.filter(item =>
    searchTerm === '' || item.title?.toLowerCase().includes(searchTerm.toLowerCase()) || item.description?.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen">
        <NavigationSidebar />
        <div className="flex-1 flex items-center justify-center text-gray-400">Loading items...</div>
      </div>
    );
  }
      ...
      <main className="flex-1 p-8 pt-16 md:pt-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-3xl font-bold gradient-text mb-1">🛒 Shopping/Watch</h1>
              <p className="text-sm text-gray-400">Track things to buy or watch</p>
            </div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-4">
            {[{ icon: '🛍️', label: 'To Buy', key: 'to-buy' }, { icon: '👀', label: 'Watching', key: 'watching' }, { icon: '📦', label: 'Archived', key: 'archived' }].map(({ icon, label, key }) => (
          ...
          <div className="flex gap-3 flex-wrap mb-6">
            <input type="text" placeholder="🔍 Search items..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="flex-1 min-w-48 bg-gray-800/50 border border-gray-600/50 rounded-lg px-4 py-2.5 text-white text-sm outline-none focus:border-purple-500" />
          ...
          <div className="glass-card rounded-2xl p-6">
            {filteredShopping.length === 0 ? (
              ...
              <div className="grid gap-4" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))' }}>
                {filteredShopping.map(item => <ShoppingCard key={item.id} item={item} onEdit={() => openEdit(item)} onDelete={() => setDeleteId(item.id)} />)}
              </div>
            )}
          </div>
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold text-white mb-4">{modal.mode === 'add' ? 'Add Item' : 'Edit Item'}</h2>
            <div className="flex flex-col gap-3">
              ...
              <input placeholder="URL" value={form.url} onChange={e => setForm(f => ({ ...f, url: e.target.value }))} className="bg-gray-800 border border-gray-600 rounded-lg px-3 py-2.5 text-white text-sm outline-none focus:border-purple-500" />
              ...
              <input placeholder="Price (e.g. $29.99)" value={form.price} onChange={e => setForm(f => ({ ...f, price: e.target.value }))} className="bg-gray-800 border border-gray-600 rounded-lg px-3 py-2.5 text-white text-sm outline-none focus:border-purple-500" />
              <input placeholder="Image URL (optional)" value={form.image} onChange={e => setForm(f => ({ ...f, image: e.target.value }))} className="bg-gray-800 border border-gray-600 rounded-lg px-3 py-2.5 text-white text-sm outline-none focus:border-purple-500" />
...
function ShoppingCard({ item, onEdit, onDelete }) {
  return (
    <div className="group bg-gray-800/70 rounded-xl border border-gray-600/50 p-4 transition-all hover:border-purple-500/50 hover:-translate-y-1 hover:shadow-lg hover:shadow-purple-900/20">
      {item.image && <img src={item.image} alt={item.title} className="w-full h-48 object-cover rounded-lg mb-3" />}
      <div className="flex items-start justify-between gap-2 mb-2">
        <a href={item.url} target="_blank" rel="noopener noreferrer" className="text-white hover:text-blue-400 text-base font-semibold block no-underline leading-snug transition-colors">
          {item.title}
        </a>
      ...
      <div className="flex gap-2 flex-wrap mb-3 items-center">
        {item.price && <span className="px-2 py-1 bg-green-700 rounded text-xs font-semibold text-white">{item.price}</span>}
        {item.category && <span className="px-2 py-1 bg-gray-900 rounded text-xs text-gray-400">{item.category}</span>}
        <span className="px-2 py-1 bg-gray-900 rounded text-xs text-gray-400">{new Date(item.created_at).toLocaleDateString()}</span>
      </div>
